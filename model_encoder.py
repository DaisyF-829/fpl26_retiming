...
class DeepGCNIIEncoder(nn.Module):
    ...
    def forward(self, g):
        ...
        if self.use_jk:
            ...
            x = self.jk(x_list)
        g.ndata['h'] = x
        graph_emb = dgl.readout_nodes(g, 'h', op='mean')
        return x, graph_emb
...
class GCNEncoder(nn.Module):
    ...
    def forward(self, g):
        ...
        if self.use_jk:
            x = self.jk(x_list)
        g.ndata['h'] = x
        graph_emb = dgl.readout_nodes(g, 'h', op='sum')
        return x, graph_emb
...
class GINEEncoder(nn.Module):
    ...
    def forward(self, g):
        ...
        # Compute global graph embedding: here we can aggregate both node and edge features
        graph_emb = dgl.readout_nodes(g, 'h', op='mean')  # Mean pooling of node features
        # Alternatively, you can include edge features in the global embedding calculation
        # edge_emb = dgl.readout_edges(g, 'feat', op='mean')  # Mean pooling of edge features
        # Combine node and edge embeddings if needed
        # graph_emb = graph_emb + edge_emb  # or any other combination strategy
